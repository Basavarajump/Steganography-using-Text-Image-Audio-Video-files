...
def encode_vid_data():
    ...
    out = cv2.VideoWriter('stego_video1.mp4',fourcc, 25.0, size)
    max_frame=0;
    while(cap.isOpened()):
        ret, frame = cap.read()
        if ret == False:
...
video = Button(root, text='video\nSteganography', relief='flat', bg='#A68064', command=vid_steg, font=cas)
video.place(x=370, y=250)
ho.CreateToolTip(audio, 'Click here\nto hide data in\n an video file.')
